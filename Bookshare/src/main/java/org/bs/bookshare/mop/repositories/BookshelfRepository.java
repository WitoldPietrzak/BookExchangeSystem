...
public interface BookshelfRepository extends JpaRepository<Bookshelf, Long> {
    ...
    @Query("SELECT DISTINCT bs FROM Bookshelf bs WHERE (( :lat is null OR :lon is null OR :distance is null) OR ( (bs.locationLat is not null AND bs.locationLong is not null ) AND " + DISTANCE_FORMULA + " < :distance )) AND ( (:bookCount is null ) OR (size(bs.booksOnShelf) >= :bookCount) )")
    ...
    @Query("SELECT DISTINCT bs FROM Bookshelf bs WHERE ( (:bookCount is null ) OR (size(bs.booksOnShelf) >= :bookCount) )")
